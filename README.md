# OMMA Stitch Service — OpenCV Backend + AI Reconstruct

Production stitching backend for the OMMA 360 Builder.  
**Memory-optimized for Render starter tier (512 MB).**

## AI Reconstruct Mode (v1.2.0)

When construction photography has incomplete overlap or limited coverage, the `ai_reconstruct` engine:

1. Attempts OpenCV stitch (preserves all original pixels)
2. Detects coverage gaps in the stitched result
3. Estimates room geometry from covered regions
4. Pads to equirectangular aspect ratio if needed
5. Fills missing areas using geometric inpainting (Telea/Navier-Stokes)
6. Returns a coverage mask so the viewer can toggle original vs AI regions

**Rules:**
- Never modifies source photographs
- AI-generated regions are tagged in the coverage mask
- Construction accuracy > visual perfection
- Geometric continuity preferred over artistic creativity
...
Server-side deployment endpoint. Reads the 5 managed backend files from disk and embedded constants, compares against GitHub, pushes changed files, waits for Render deploy, and verifies both `/api/health` and `/api/ai-reconstruct`.

**Request:**
```json
{
  "githubToken": "ghp_xxxxxxxxxxxx",
  "repo": "joetechsupport/360view",
  "branch": "main",
  "verifyUrl": "https://three60view-noz3.onrender.com",
  "commitMessage": "Auto-update stitch backend"
}
```
...
```json
{
  "success": true,
  "fileStatuses": [
    {"filename": "main.py", "status": "pushed"},
    {"filename": "requirements.txt", "status": "unchanged"},
    {"filename": "Dockerfile", "status": "unchanged"},
    {"filename": "render.yaml", "status": "unchanged"},
    {"filename": "README.md", "status": "pushed"}
  ],
  "pushedCount": 2,
  "unchangedCount": 3,
  "errorCount": 0,
  "healthVerified": true,
  "aiReconstructVerified": true,
...
Same request format as /api/stitch. Returns additional fields:
...
```json
{
  ...
  "originalCoveragePercent": 78.3,
  "aiGeneratedPercent": 21.7,
  "coverageMaskBase64": "...",
  "reconstructionMethod": "geometric_inpaint",
  "warnings": ["AI reconstruction assisted — 21.7% synthesized, 78.3% original photography"],
